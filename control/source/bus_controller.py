...
class BusController(SerialControl):
    ...
    def __init__(
        ...
        # self.logger = logging.getLogger(f"{self.__class__.__name__}({bus_port})")
    ...
    def _read_serial_response(self) -> IncomingMessage:
        while self.connection.in_waiting < EXAMPLE_INCOMING_MESSAGE.packet_size:
            time.sleep(0.1)
        self.logger.debug(f"Data ready to read - {self.connection.in_waiting}")
